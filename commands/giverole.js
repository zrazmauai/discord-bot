module.exports = {
  name: `giverole`,
  aliases: [`gr`, `giver`, `grole`],
  description: `This command grants one or more existing roles to one or more users.`,
  syntax: `--giverole\` \`@user1 @user2 [...]\` \`/\` \`@role1 @role2 [...]\` **or** \`rolename1 / rolename2 / [...]`,

  // CODE BELOW THIS LINE

  execute(BOT, message, ARGS, DISCORD) {
    // STEOP [00] - Checks if the command caller has the necessary permissions to manage the server's roles.
    ...
    for (let i = 0; i < ROLES_ARRAY.length; i++) {
      if (ROLES_ARRAY[i].managed) {
        return message.channel.send(
          `I'm sorry, but it seems at least one of the specified roles is **managed by an external application** and therefore **cannot be attributed to any user.**`
        );
      }
    }
      ...
      for (let i = 0; i < USERS_ARRAY.length; i++) {
        if (
          ...
          message.channel.send(
            `The user ${USERS_ARRAY[i]} already has the role ${roleElement}.`
          );
        } else {
          MEMBERS_ARRAY[i].roles.add(roleElement).catch();
          message.channel.send(
            `The user ${USERS_ARRAY[i]} has been successfully added to the role ${roleElement}.`
          );
        }
      }
    });
